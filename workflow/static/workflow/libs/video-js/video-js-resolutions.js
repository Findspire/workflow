...
videojs.plugin('resolutions', function(options) {
  ...
  player.fsUpdateResolution = function(source) {
      var $content = $(this.controlBar.el_).find(".vjs-resolutions-button .vjs-control-content"),
          source_res = source["data-res"],
          is_hd = (source_res === "HD");

    if (is_hd) {
        $content.addClass("vjs-resolution-hd");
    } else {
        $content.removeClass("vjs-resolution-hd");
    }
    $content.attr("data-resolution", source_res);
  };
  ...
  player.changeResolution = function(new_source){
    this.fsUpdateResolution(new_source);
  ...
  player.ready(function(){
    ...
    this.fsUpdateResolution(source);
  });
});
